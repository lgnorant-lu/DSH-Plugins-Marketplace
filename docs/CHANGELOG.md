...
- **Git Hooks / 日志 / 性能模块测试覆盖补齐（重构优化，无新特性）**：`hook-check.test.mjs` 补 syntax/toc/commit-msg 实际执行覆盖（原仅源码正则断言）；`app-diagnostics.test.mjs` 补事件字段边界归一化（非数字 duration→NaN、超长 message→截断、非字符串 level/error_code/trace_id→String）；`benchmark-contract.test.mjs` 补实际运行输出合法 JSON 断言（含全部规模维度 + median/p95/samples + git revision）/ test coverage for the Git Hooks, diagnostics event ring, and performance benchmark modules is completed (refactor/optimization only, no new features): hook-check gains real execution coverage for syntax/toc/commit-msg; app-diagnostics gains event-field edge normalization assertions; benchmark-contract gains a live-run valid-JSON assertion
- **coverage 性能优化（重构优化，无新特性）**：coverage.mjs 改为只跑 unit+integration（2m41s→58s），并把此前 e2e 独享的 10 个函数分移到 unit/integration 补测试（validation env 键纯函数、skills 200 成功分支 incl. q 过滤与 pkg 去重、annotateSkillInstalled/getInstalledEntries 闭包），5 个真实深集成/防御死代码函数加入豁免（now 默认参数、readFile 防御 catch、buildPluginPackage 真实 npm/pnpm 构建仍由 e2e 覆盖）；coverage 570/570 (100%) 保持，门快速通过 / coverage.mjs now runs unit+integration only (2m41s→58s), moving 10 e2e-exclusive branches into unit/integration (validation env-key pure functions, the skills 200 success path incl. q-filter and pkg-dedupe, the annotateSkillInstalled/getInstalledEntries closures) and exempting 5 deep-integration/defensive-dead-code functions (now default params, readFile defense catches, buildPluginPackage real npm/pnpm build still covered by e2e); coverage stays 570/570 (100%) while the gate runs fast
