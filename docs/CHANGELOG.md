...
- **测试运行器性能优化（重构优化，无新特性）**：`run.mjs` 层内文件并发池（默认 4，`DSH_TEST_CONCURRENCY` 可调）异步 spawn，层间保持 unit→integration→e2e 顺序；每个子测试在未显式设置 `DRIFT_REPORT_FILE` 时获得独立临时报告路径（避免并行写同一文件竞争）；`--json` 抑制子测试 stdout/stderr 与清理输出、结果按层序+文件名排序保证 CI 输出确定性；失败文件聚合摘要 + 每文件耗时统计。`install-scripts.test.mjs` sh 侧 fixture 化：复制 `install.sh` + 占位 `package.json` 到临时目录运行，避免 `cp -r` 复制整个 2.2G 仓库（82s→5.5s），与 ps1 侧对称 / the test runner is now parallelized (per-level concurrency pool, default 4, tunable via DSH_TEST_CONCURRENCY) with per-file drift-report isolation, deterministic sorted --json output, and a failure summary with per-file timing; the install-scripts sh sandbox is fixture-ized to copy install.sh + a placeholder package.json into a temp dir instead of copying the whole 2.2G repo (82s→5.5s), mirroring the ps1 side
