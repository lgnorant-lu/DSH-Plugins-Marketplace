window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DICT_ZH = {
      ...
      fbToggle: "是否发送反馈",
      fbAskFailed: "上次安装 {name} 失败，要提交反馈吗？",
      fbSubmitReport: "提交反馈",
      fbRetry: "重试安装",
      fbNoAsk: "不再询问",
      fbManualLog: "已打开 GitHub 反馈页（安装日志已复制到剪贴板，可粘贴补充）",
      fbModeAsk: "每次询问",
      fbModeFailed: "仅失败时询问",
      fbModeOff: "关闭",
      backupExport: "导出备份",
      ...
      cliHintCopy: "复制",
      cliHintCopied: "已复制 ✔",
      badgeBundle: "bundle 插件",
      badgeBundleTip: "经 profile bundles 层注册的 bundle 形态（dsh.bundle.patch 声明），安装走 bundle 专用管线",
      badgeDesktop: "桌面客户端",
      badgeDesktopTip: "安装器/桌面客户端/启动器形态——非插件生态件，市场无法代装",
      badgeArchived: "已归档",
      badgeArchivedTip: "该 GitHub 仓库已归档，可能不再维护；安装前请确认仍适用",
      typeCli: "官方 CLI",
      typeMap: {
    ...
    var DICT_EN = {
      ...
      fbAskFailed: "Installation of {name} failed. Submit feedback?",
      fbSubmitReport: "Submit feedback",
      fbRetry: "Retry install",
      fbNoAsk: "Don't ask again",
      fbManualLog: "GitHub feedback page opened (install log copied to clipboard — paste to attach)",
      fbModeAsk: "Ask every time",
      fbModeFailed: "Ask on failure only",
      fbModeOff: "Off",
      ...
      badgeBundle: "bundle plugin",
      badgeBundleTip: "bundle form registered via the profile bundles layer (dsh.bundle.patch); installs through the bundle pipeline",
      badgeDesktop: "desktop client",
      badgeDesktopTip: "installer / desktop client / launcher form — not a plugin; the marketplace cannot install it",
      badgeArchived: "archived",
      badgeArchivedTip: "This GitHub repository is archived and may no longer be maintained; verify it is still suitable before installing",
    ...
    // 写操作请求头：CSRF 头 + 会话 token（服务端 tapIndex 注入页面，
    ...
    var s = {
      ...
      badgeBundle: { display: "inline-block", padding: "1px 8px", borderRadius: 10, fontSize: 11, border: "1px solid var(--dsw-alias-state-success-secondary, #15803d)", color: "var(--dsw-alias-state-success-primary)", marginLeft: 8, cursor: "default", textTransform: "none" },
      badgeDesktop: { display: "inline-block", padding: "1px 8px", borderRadius: 10, fontSize: 11, border: "1px solid var(--dsw-alias-state-warn-secondary, #b45309)", color: "var(--dsw-alias-state-warn-primary)", marginLeft: 8, cursor: "default", textTransform: "none" },
      badgeArchived: { display: "inline-block", padding: "1px 8px", borderRadius: 10, fontSize: 11, border: "1px solid var(--dsw-alias-state-warn-secondary, #b45309)", color: "var(--dsw-alias-state-warn-primary)", marginLeft: 8, cursor: "default", textTransform: "none" },
      ...
      toast: { position: "fixed", top: 24, left: "50%", transform: "translateX(-50%)", zIndex: 2200, maxWidth: "80vw", padding: "8px 16px", borderRadius: 10, fontSize: 13, boxShadow: "var(--dsw-shadow-lv3, 0 8px 24px rgba(0,0,0,.25))", background: "var(--dsw-alias-button-contrast-fill)", color: "var(--dsw-alias-label-primary-inverted)" },
      toastErr: { position: "fixed", top: 24, left: "50%", transform: "translateX(-50%)", zIndex: 2200, maxWidth: "80vw", padding: "8px 16px", borderRadius: 10, fontSize: 13, boxShadow: "var(--dsw-shadow-lv3, 0 8px 24px rgba(0,0,0,.25))", background: "var(--dsw-alias-state-error-primary)", color: "var(--dsw-alias-label-primary-inverted)" }
    };
    ...
    function PluginTab(props) {
      ...
      return h("div", null,
        ...
        repos ? (function () {
          var q = query.trim().toLowerCase();
          var list = filterRepos(repos, query, category);
    ...
    function SkillsTab(props) {
          ...
          .then(function (data) {
            ...
            if (data.error) { setError(data.error); setLoading(false); }
              ...
              setRepos(function (prev) {
                return appendSkillsPage(prev, pg, data.repos);
              });
              setPage(pg);
      ...
      useEffect(function () {
        ...
        var observer = new IntersectionObserver(function (entries) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting && shouldLoadMore(repos, total, loading)) {
